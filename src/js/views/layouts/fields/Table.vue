<template>
    <div class="i-table  mb-2 overflow-x-auto">
        ...
        <component v-if="getSlots('columns')" :is="getSlots('columns')" :store-namespace="storeNamespace"></component>
        <div v-else-if="columns" class="d-flex">
            <div class="col-1"></div>
            <div v-if="getOption('fields.prepend')" class="px-0" v-for="item in getOption('fields.prepend')" :class="item.style" v-text="item.title"></div>
            ...
            <div class="pr-4 mb-2" :class="getStyle('column')" v-for="(column, key) in getColumns">
                <div v-if="getOption('fields.columns')" class="form-group row mb-0">
                    <div v-for="item in getOption('fields.columns')" :class="getStyle('item')">
                        <component :is="item.component" v-bind="item.attrs" :field-index="fieldIndex + '.'  + getOption('prefix.row')+ '.' + index + '.' + item.name + '.' + getOption('prefix.column') + '.' + key" :store-namespace="storeNamespace"></component>
                        <div class="d-md-none mb-3"></div>
                    </div>
                </div>
            </div>
            <div v-if="getOption('fields.append')" v-for="item in getOption('fields.append')" class="p-0 pr-4" :class="item.style">
                <component :is="item.component" v-bind="item.attrs" :field-index="fieldIndex + '.' + getOption('prefix.row')+ '.' + index + '.' + item.name" :store-namespace="storeNamespace"></component>
            </div>
        </div>
    </div>
