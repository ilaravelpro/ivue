<template>
    <div class="d-flex flex-wrap input-group">
        <i-base-autocomplete class="col-4"  placeholder="Enter Country" :items="countries" @change="model.country = $event" :release="(country, index) => {return {text: `+${country.code}`, value: country.code}}"/>
        <i-base-input class="col-8" @change="model.number = $event"  placeholder="Enter Mobile"/>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            countryCode() {
                var $this = this;
                var $country = this.allCountries.filter((row) => {
                    return row[2] === String($this.defCountry || $this.model.country);
                });
                return $country[0] ? String($country[0][1]).toUpperCase() : null
            }
        },
        methods: {
            ...GlobalField.methods(),
        ...
        watch: {
            ...
            /*model: {
                handler: function (newValue, oldValue) {
                    if (newValue === null){
                        this.model = oldValue;
                    }else if (newValue != null && !(this.getIndex('store') &&
                            this.getOption('store.update', true)) && newValue !== this.value)
                            this.$emit('change', newValue);
                    if (newValue) this.phone.number = String(newValue.number);
                },
                deep: true
            },*/
        },
